	...
	for _, executor := range app.Spec.Executor {
		if numInitialExecutors := util.GetInitialExecutorNumber(app, executor); numInitialExecutors > 0 {
			...
			taskGroups = append(taskGroups, taskGroup{
				...
				NodeSelector: mergeNodeSelector(app.Spec.NodeSelector, executor.NodeSelector),
		...
		executor.SchedulerName = util.StringPtr(SchedulerName)
	}
...
func addTaskGroupAnnotations(app *v1beta2.SparkApplication, taskGroups []taskGroup) error {
	...
	for _, executor := range app.Spec.Executor {
		...
		executor.Annotations[taskGroupNameAnnotation] = executorTaskGroupName
...
func addQueueLabels(app *v1beta2.SparkApplication) {
	if app.Spec.BatchSchedulerOptions != nil && app.Spec.BatchSchedulerOptions.Queue != nil {
		...
		for _, executor := range app.Spec.Executor {
			if executor.Labels == nil {
				executor.Labels = make(map[string]string)
			}

			executor.Labels[queueLabel] = *app.Spec.BatchSchedulerOptions.Queue
